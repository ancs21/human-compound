...
Scan `.context/human-compound/todos/*.md` and legacy `todos/*.md`. Partition by status:
...
Spawn a `human-compound:workflow:pr-comment-resolver` agent per item. Prefer parallel; fall back to sequential respecting dependency order.
...
For large sets, use a scratch directory at `.context/human-compound/todo-resolve/<run-id>/` for per-resolver artifacts. Return only completion summaries to parent.
...
Load the `he:compound` skill to document what was learned. Todo resolutions often surface patterns and architectural insights worth capturing.
...
Delete completed/resolved todo files from both paths. If a scratch directory was created at `.context/human-compound/todo-resolve/<run-id>/`, delete it (unless user asked to inspect).
