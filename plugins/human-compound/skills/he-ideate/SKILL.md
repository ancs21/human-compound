...
- a path such as `plugins/human-compound/skills/`
...
Before starting, search GitHub Issues for existing team proposals on this topic:

```bash
gh issue list --search "<focus keywords>" --label "ideation" --state open --limit 5
```

If relevant Issues exist:
- Summarize what the team has already proposed
- Use team discussion as input to ideation rather than starting from scratch
- Avoid duplicating ideas the team has already explored or rejected

If no Issues exist, proceed normally.
...
2. **Learnings search** — dispatch `human-compound:research:learnings-researcher` with a brief summary of the ideation focus.

3. **Issue intelligence** (conditional) — if issue-tracker intent was detected in Phase 0.2, dispatch `human-compound:research:issue-intelligence-analyst` with the focus hint. If a focus hint is present, pass it so the agent can weight its clustering toward that area. Run this in parallel with agents 1 and 2.
...
- the workflow is about to hand off to `he:brainstorm`, Proof sharing, or session end
...
- acting on an idea routes to `he:brainstorm`, not directly to implementation
