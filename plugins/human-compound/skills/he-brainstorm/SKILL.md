...
Brainstorming helps answer **WHAT** to build through collaborative dialogue. It precedes `/he:plan`, which answers **HOW** to build it.
...
#### 0.0 Check Team Context (human-compound)

Before starting, search GitHub Issues for existing team discussion on this topic:

```bash
gh issue list --search "<topic keywords>" --state open --limit 5
```

If relevant Issues exist:
- Summarize what the team has already discussed
- Link the Issue number in the requirements doc when one is created
- Carry forward any decisions or constraints the team already agreed on

If no Issues exist, suggest opening one so the team has visibility: "Consider opening a GitHub Issue so your team can see this brainstorm is underway."
...
## Next Steps
[If `Resolve Before Planning` is empty: `→ /he:plan` for structured implementation planning]
...
- What would `he:plan` still have to invent if this brainstorm ended now?
...
- **Proceed to planning (Recommended)** - Run `/he:plan` for structured implementation planning
...
Immediately run `/he:plan` in the current session. Pass the requirements document path when one exists; otherwise pass a concise summary of the finalized brainstorm decisions. Do not print the closing summary first.

**If user selects "Proceed directly to work":**

Immediately run `/he:work` in the current session using the finalized brainstorm output as context. If a compact requirements document exists, pass its path. Do not print the closing summary first.
...
Resume with `/he:brainstorm` when ready to resolve these before planning.
```
