...
- **Write a run artifact** under `.context/human-compound/ce-review/<run-id>/` summarizing findings, applied fixes, residual actionable work, and advisory outputs.
...
- **Never edit files or externalize work.** Do not write `.context/human-compound/ce-review/<run-id>/`, do not create todo files, and do not commit, push, or create a PR.
...
- **Write a run artifact** under `.context/human-compound/ce-review/<run-id>/` summarizing findings, applied fixes, and advisory outputs. Include the artifact path in the structured output.
...
| Agent | Focus |
|-------|-------|
| `human-compound:review:correctness-reviewer` | Logic errors, edge cases, state bugs, error propagation |
| `human-compound:review:testing-reviewer` | Coverage gaps, weak assertions, brittle tests |
| `human-compound:review:maintainability-reviewer` | Coupling, complexity, naming, dead code, abstraction debt |
| `human-compound:review:project-standards-reviewer` | CLAUDE.md and AGENTS.md compliance -- frontmatter, references, naming, portability |
| `human-compound:research:learnings-researcher` | Search docs/solutions/ for past issues related to this PR |

**Cross-cutting conditional (selected per diff):**

| Agent | Select when diff touches... |
|-------|---------------------------|
| `human-compound:review:security-reviewer` | Auth, public endpoints, user input, permissions |
| `human-compound:review:performance-reviewer` | DB queries, data transforms, caching, async |
| `human-compound:review:api-contract-reviewer` | Routes, serializers, type signatures, versioning |
| `human-compound:review:data-migrations-reviewer` | Migrations, schema changes, backfills |
| `human-compound:review:reliability-reviewer` | Error handling, retries, timeouts, background jobs |
| `human-compound:review:adversarial-reviewer` | Diff >=50 changed non-test/non-generated/non-lockfile lines, or auth, payments, data mutations, external APIs |
| `human-compound:review:previous-comments-reviewer` | Reviewing a PR that has existing review comments or threads |

**CE conditional (migration-specific):**

| Agent | Select when diff includes migration files |
|-------|------------------------------------------|
| `human-compound:review:deployment-verification-agent` | Produces deployment checklist with SQL verification queries |

## Review Scope

Every review spawns all 4 always-on personas plus the learnings-researcher, then adds whichever cross-cutting conditionals fit the diff. The model naturally right-sizes: a small config change triggers 0 conditionals = 5 reviewers. A feature touching auth and queries might trigger security + reliability + performance = 8 reviewers.

## Protected Artifacts

The following paths are human-compound pipeline artifacts and must never be flagged for deletion, removal, or gitignore by any reviewer:
...
### Stage 1b: Check for Human Review Comments (human-compound)

Before running AI review agents, check if teammates have already left PR comments:

```bash
gh pr view --json comments,reviews --jq '.comments[].body, .reviews[].body'
```

If human review comments exist:
- Summarize them briefly: "Your teammates have already commented on: [topics]"
- Feed human comments into the AI review context so agents don't duplicate feedback the team already gave
- If human reviewers flagged a critical issue, prioritize it in the findings

This ensures AI review complements human review rather than repeating it.
...
2. Filter to those whose directory is an ancestor of at least one changed file. A standards file governs all files below it (e.g., `plugins/human-compound/AGENTS.md` applies to everything under `plugins/human-compound/`).
...
CE always-on agents (learnings-researcher) and CE conditional agents (deployment-verification-agent) also use the cheaper model tier since they perform scoped, focused work.
...
7. **Agent learnings:** If the agent's `.md` file contains a `## Learnings` section with recorded entries (not just "No learnings recorded yet"), include those learnings in the agent prompt wrapped in a `<agent-learnings>` block. These are calibration corrections from past reviews — the agent must respect them to avoid repeating known false positives or missed patterns
...
**CE always-on agents** (learnings-researcher) are dispatched as standard Agent calls in parallel with the persona agents. Give them the same review context bundle the personas receive: entry mode, any PR metadata gathered in Stage 1, intent summary, review base branch name when known, `BASE:` marker, file list, diff, and `UNTRACKED:` scope notes. Do not invoke them with a generic "review this" prompt. Their output is unstructured and synthesized separately in Stage 6.
...
Artifact: .context/human-compound/ce-review/<run-id>/
...
- In interactive, autofix, and headless modes, write a per-run artifact under `.context/human-compound/ce-review/<run-id>/` containing:
...
**Agent learning prompt (interactive mode only):** If the user dismissed any findings during the fix-review cycle (marked as false positive, wrong severity, or not applicable), suggest: "Some findings were dismissed. Record them as agent learnings so they don't repeat? (`/he:agent-learn`)" If the user agrees, invoke the `he:agent-learn` skill with the dismissed findings, routing each to the correct agent.
