...
`he:brainstorm` defines **WHAT** to build. `he:plan` defines **HOW** to build it. `he:work` executes the plan.

This workflow produces a durable implementation plan. It does **not** implement code, run tests, or learn from execution-time results. If the answer depends on changing code and seeing what happens, that belongs in `he:work`, not here.
...
- If the ambiguity is mainly product framing, user behavior, or scope definition, recommend `he:brainstorm` first
...
- Ask the user, using the platform's blocking question tool when available (see Interaction Method), whether to:
  1. Resume `he:brainstorm` to resolve them
...
- Task human-compound:research:repo-research-analyst(Scope: technology, architecture, patterns. {planning context summary})
- Task human-compound:research:learnings-researcher(planning context summary)
...
- Task human-compound:research:best-practices-researcher(planning context summary)
- Task human-compound:research:framework-docs-researcher(planning context summary)
...
- Task human-compound:workflow:spec-flow-analyzer(planning context summary, research findings)
...
- The plan does not invent product behavior that should have been defined in `he:brainstorm`
...
- Blocking questions were either resolved, explicitly assumed, or sent back to `he:brainstorm`
...
#### 5.2b Share Plan with Team (human-compound)

After writing the plan, offer to create a PR so the team can review it:

"Plan written. Create a PR for team review?"

If yes:
```bash
git checkout -b plan/<descriptive-name>
git add docs/plans/<filename>
git commit -m "docs(plan): <plan title>"
git push -u origin plan/<descriptive-name>
gh pr create --title "Plan: <plan title>" --body "$(cat <<'EOF'
## Plan for team review

See `docs/plans/<filename>` for the full plan.

**Key decisions:**
- [Decision 1]
- [Decision 2]

**Implementation units:** [N] units
...
Link to the originating GitHub Issue if one exists.
...
- `human-compound:workflow:spec-flow-analyzer` for missing user flows, edge cases, and handoff gaps
- `human-compound:research:repo-research-analyst` (Scope: `architecture, patterns`) for repo-grounded patterns, conventions, and implementation reality checks

**Context & Research / Sources & References gaps**
- `human-compound:research:learnings-researcher` for institutional knowledge and past solved problems
- `human-compound:research:framework-docs-researcher` for official framework or library behavior
- `human-compound:research:best-practices-researcher` for current external patterns and industry guidance
- Add `human-compound:research:git-history-analyzer` only when historical rationale or prior art is materially missing

**Key Technical Decisions**
- `human-compound:review:architecture-strategist` for design integrity, boundaries, and architectural tradeoffs
- Add `human-compound:research:framework-docs-researcher` or `human-compound:research:best-practices-researcher` when the decision needs external grounding beyond repo evidence

**High-Level Technical Design**
- `human-compound:review:architecture-strategist` for validating that the technical design accurately represents the intended approach and identifying gaps
- `human-compound:research:repo-research-analyst` (Scope: `architecture, patterns`) for grounding the technical design in existing repo patterns and conventions
- Add `human-compound:research:best-practices-researcher` when the technical design involves a DSL, API surface, or pattern that benefits from external validation

**Implementation Units / Verification**
- `human-compound:research:repo-research-analyst` (Scope: `patterns`) for concrete file targets, patterns to follow, and repo-specific sequencing clues
- `human-compound:review:pattern-recognition-specialist` for consistency, duplication risks, and alignment with existing patterns
- Add `human-compound:workflow:spec-flow-analyzer` when sequencing depends on user flow or handoff completeness

**System-Wide Impact**
- `human-compound:review:architecture-strategist` for cross-boundary effects, interface surfaces, and architectural knock-on impact
- Add the specific specialist that matches the risk:
  - `human-compound:review:performance-oracle` for scalability, latency, throughput, and resource-risk analysis
  - `human-compound:review:security-sentinel` for auth, validation, exploit surfaces, and security boundary review
  - `human-compound:review:data-integrity-guardian` for migrations, persistent state safety, consistency, and data lifecycle risks

**Risks & Dependencies / Operational Notes**
- Use the specialist that matches the actual risk:
  - `human-compound:review:security-sentinel` for security, auth, privacy, and exploit risk
  - `human-compound:review:data-integrity-guardian` for persistent data safety, constraints, and transaction boundaries
  - `human-compound:review:data-migration-expert` for migration realism, backfills, and production data transformation risk
  - `human-compound:review:deployment-verification-agent` for rollout checklists, rollback planning, and launch verification
  - `human-compound:review:performance-oracle` for capacity, latency, and scaling concerns
...
Artifact-backed mode uses a per-run scratch directory under `.context/human-compound/ce-plan/deepen/`.
...
- Recommend `he:brainstorm` if the gap is truly product-defining
...
**Options:**
1. **Start `/he:work`** - Begin implementing this plan in the current environment (recommended)
2. **Open plan in editor** - Open the plan file for review
3. **Run additional document review** - Another pass for further refinement
4. **Start `/he:work` in another session** - Begin implementing in a separate agent session when the current platform supports it
...
- **`/he:work`** → Call `/he:work` with the plan path
- **`/he:work` in another session** → If the current platform supports launching a separate agent session, start `/he:work` with the plan path there. Otherwise, explain the limitation briefly and offer to run `/he:work` in the current session instead.
...
- Ask whether to proceed to `/he:work`
