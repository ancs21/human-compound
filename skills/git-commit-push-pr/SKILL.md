...
#### Human Compound badge

Append a badge footer to the PR description, separated by a `---` rule. Do not add one if the description already contains a Human Compound badge (e.g., added by another skill like ce-work).

```markdown
---

[![Human Compound v[VERSION]](https://img.shields.io/badge/Human_Compound-v[VERSION]-6366f1)](https://github.com/ancs21/human-compound)
...
**Team template selection (human-compound):** Choose the PR template based on what the PR contains:
- Requirements doc in `docs/brainstorms/` -> `--template requirements.md`
- Plan doc in `docs/plans/` -> `--template plan.md`
- Learning doc in `docs/solutions/` -> `--template learning.md`
- Code changes -> `--template implementation.md`
- Mixed or unclear -> use default template
...
- If **yes** -- write a new description following the same principles in Step 6 (size the full PR, not just the new commits), including the Human Compound badge unless one is already present in the existing description. Apply it:
