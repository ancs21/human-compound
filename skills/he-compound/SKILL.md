...
```bash
/he:compound                    # Document the most recent fix
/he:compound [brief context]    # Provide additional context hint
```
...
**Always run full mode by default.** Proceed directly to Phase 1 unless the user explicitly requests compact-safe mode (e.g., `/he:compound --compact` or "use compact mode").
...
7. **Share with team (human-compound)**

   After writing the learning doc, offer to create a PR so the team sees it:

   "Learning captured. Create a PR so the team can see it?"

   If yes:
   ```bash
   git checkout -b learning/<problem-slug>
   git add docs/solutions/<category>/<filename>
   git commit -m "docs(learning): <problem title>"
   git push -u origin learning/<problem-slug>
   gh pr create --title "Learning: <problem title>" --body "$(cat <<'PREOF'
   ## Learning captured

   **Problem:** <1-line summary>
   **Solution:** <1-line summary>

   See `docs/solutions/<category>/<filename>` for the full doc.
   ...
   Link to the originating Issue or PR if one exists.
...
`he:compound-refresh` is **not** a default follow-up. Use it selectively when the new learning suggests an older learning or pattern doc may now be inaccurate.

It makes sense to invoke `he:compound-refresh` when one or more of these are true:
...
It does **not** make sense to invoke `he:compound-refresh` when:
...
- If there is **one obvious stale candidate**, invoke `he:compound-refresh` with a narrow scope hint after the new learning is written
- If there are **multiple candidates in the same area**, ask the user whether to run a targeted refresh for that module, category, or pattern set
- If context is already tight or you are in compact-safe mode, do not expand into a broad refresh automatically; instead recommend `he:compound-refresh` as the next step with a scope hint

When invoking or recommending `he:compound-refresh`, be explicit about the argument to pass. Prefer the narrowest useful scope:
...
Do not invoke `he:compound-refresh` without an argument unless the user explicitly wants a broad sweep.
...
In compact-safe mode, the overlap check is skipped (no Related Docs Finder subagent). This means compact-safe mode may create a doc that overlaps with an existing one. That is acceptable — `he:compound-refresh` will catch it later. Only suggest `he:compound-refresh` if there is an obvious narrow refresh target. Do not broaden into a large refresh sweep from a compact-safe session.
...
Specialized Agent Reviews (Auto-Triggered):
  ...
  ✓ correctness-reviewer: Code examples are correct
...
<manual_override> Use /he:compound [context] to document immediately without waiting for auto-detection. </manual_override> </auto_invoke>
...
### Code Quality & Review
- **correctness-reviewer**: Reviews code examples for correctness
...
- **Manual trigger**: User can invoke agents after /he:compound completes for deeper review

## Related Commands

- `/research [topic]` - Deep investigation (searches docs/solutions/ for patterns)
- `/he:plan` - Planning workflow (references documented solutions)
