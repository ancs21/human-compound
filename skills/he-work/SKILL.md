---
name: he:work
description: "Execute work with external delegate support. Includes Codex delegation mode for token-conserving code implementation."
...
2. **Assess complexity and route**
   ...
   | **Large** | Cross-cutting, architectural decisions, 10+ files, touches auth/payments/migrations | Inform the user this would benefit from `/he:brainstorm` or `/he:plan` to surface edge cases and scope boundaries. Honor their choice. If proceeding, build a task list and continue to Phase 1 step 2 |
...
6. **Figma Design Sync** (if applicable)
   ...
   - Compare implementation against design specs visually
...
2. **Code Review** (REQUIRED)
   ...
   **Tier 2: Full review (default)** — REQUIRED unless Tier 1 criteria are explicitly met. Invoke the `he:review` skill with `mode:autofix` to run specialized reviewer agents, auto-apply safe fixes, and surface residual work as todos. When the plan file path is known, pass it as `plan:<path>`. This is the mandatory default — proceed to Tier 1 only after confirming every criterion below.
...
2. **Commit and Create Pull Request**
   ...
   **Team traceability (human-compound):** Always link back to upstream artifacts:
   - Link to the plan doc: `Plan: docs/plans/<filename>`
   - Link to the originating GitHub Issue: `Closes #<number>` or `Related: #<number>`
   - Link to the requirements doc if one exists: `Requirements: docs/brainstorms/<filename>`
   - Use the `implementation.md` PR template when available: `--template implementation.md`
...
- A plan implementation unit contains `Execution target: external-delegate` in its Execution note (set by he:plan)
...
- Run project linting before pushing
...
- [ ] Code review completed (inline self-review or full `he:review`)
...
**Tier 2 (full review)** — REQUIRED default. Invoke `he:review mode:autofix` with `plan:<path>` when available. Safe fixes are applied automatically; residual work surfaces as todos. Always use this tier unless all four Tier 1 criteria are explicitly confirmed.
